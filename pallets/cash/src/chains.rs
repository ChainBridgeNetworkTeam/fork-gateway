...
impl ChainAccountSignature {
    ...
    //  CHainAccountSignature中恢复链上账号，不支持dot和gateway
...
impl ChainBlocks {
    ...
    {
        //  区块的hash和pending块的hash比较，是否有支持者
        //  已经被处理过的直接过滤
        blocks
            .into_iter()
            .filter(|block| {
...
//  判断签名者是否拥有制定验证节点集合的绝对多数，实际上是2/3
...
impl ChainBlockEvents {
    ...
    pub fn position(&self, event: &ChainBlockEvent) -> Option<usize> {
        ...
        //  区块和区块编号都一致的那个，返回的是位置
        eth_block_events
            .iter()
            .position(|(b, e)| *b == *block_num && *e == *eth_block)
    }

    //  其实还是根据内容match类型，然后移除
...
impl Chain for Ethereum {
    ...
    //  runtime_interfaces 感觉这个东西里面有干货
    fn sign_message(message: &[u8]) -> Result<Self::Signature, Reason> {
